...
use app\system\dao\SystemRouteDao;
...
class SystemRouteServices extends BaseServices
{
     ...
     * @date 2023/05/06
     */
    public function getTreeList(string $appName = 'admin', string $name = '')
    {
            ...
            foreach ($list as $key => $item) {
                if (!empty($item['children'])) {
                    foreach ($item['children'] as $k => $v) {
                        if (isset($v['store_category_id']) && isset($v['method'])) {
                            if ($v['method'] === 'DELETE') {
                                $v['method'] = 'DEL';
                            }
                            $v['pid'] = $v['store_category_id'];
     ...
     * @date 2023/4/6
     */
    public function getRouteListAll(string $app = 'admin')
    {
        //获取所有的路由
        $this->app = app();
     ...
     * @date 2023/4/6
     */
    public function syncRoute(string $app = 'admin')
    {
     ...
     * @date 2023/4/7
     */
    public function getFrom(int $id = 0, string $appName = 'admin')
    {
        ...
        $rule = [
            FormBuilder::cascader('store_category_id', '分类', $routeInfo['store_category_id'] ?? 0)->data($cateList),
