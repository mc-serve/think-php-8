...
use app\system\dao\log\SystemFileDao;
...
use app\mc\services\FileService as FileClass;
...
class SystemFileServices extends BaseServices
{
    ...
    public function getFileList()
    {
        ...
        $key = 'system_file_app_mc_public';
        $arr = CacheService::get(md5($key));
        if (!$arr) {
            $app = $this->getDir($rootPath . 'app');
            $extend = $this->getDir($rootPath . 'mc');
        ...
        if (empty($file)) {
            ...
            foreach ($arr as $ko => $vo) {
                ...
                $cha[] = [
                    ...
                    'cthash' => $cthash,
                    'atime' => $update_time['atime'],
                    'mtime' => $update_time['mtime'],
                    'ctime' => $update_time['ctime'],
                    'type' => '新增的',
                ];
