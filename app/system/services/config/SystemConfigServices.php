...
class SystemConfigServices extends BaseServices
{
    ...
    public function createTextForm(string $type, array $data)
    {
        ...
        switch ($type) {
            case 'number':
                ...
                $formbuser_ider[] = $this->builder->number($data['menu_name'], $data['info'], (float)$data['value'])->controls(false)->appendRule('suffix', [
                    'type' => 'div',
                    'class' => 'tips-info',
                ...
                break;
            case 'dateTime':
                $formbuser_ider[] = $this->builder->dateTime($data['menu_name'], $data['info'], $data['value'])->appendRule('suffix', [
                    'type' => 'div',
                    'class' => 'tips-info',
                    'domProps' => ['innerHTML' => $data['desc']]
                ]);
            ...
            case 'date':
                ...
                $formbuser_ider[] = $this->builder->date($data['menu_name'], $data['info'], $data['value'])->appendRule('suffix', [
                    'type' => 'div',
                    'class' => 'tips-info',
                    'domProps' => ['innerHTML' => $data['desc']]
                ]);
            ...
            case 'time':
                $data['value'] = json_decode($data['value'], true) ?: '';
                $formbuser_ider[] = $this->builder->time($data['menu_name'], $data['info'], $data['value'])->appendRule('suffix', [
                    'type' => 'div',
                    'class' => 'tips-info',
                    'domProps' => ['innerHTML' => $data['desc']]
                ]);
            ...
            case 'color':
                ...
                $formbuser_ider[] = $this->builder->color($data['menu_name'], $data['info'], $data['value'])->appendRule('suffix', [
                    'type' => 'div',
                    'class' => 'tips-info',
                    'domProps' => ['innerHTML' => $data['desc']]
                ]);
            ...
            default:
                ...
                if ($data['menu_name'] == 'api' || $data['menu_name'] == 'routine_api') {
                    $formbuser_ider[] = $this->builder->input($data['menu_name'], $data['info'], strpos($data['value'], 'http') === false ? sys_config('site_url') . $data['value'] : $data['value'])->appendRule('suffix', [
                        'type' => 'div',
                        'class' => 'tips-info',
                        'domProps' => ['innerHTML' => $data['desc']]
                    ])->col(13)->readonly(true);
                } else {
                    $formbuser_ider[] = $this->builder->input($data['menu_name'], $data['info'], $data['value'])->appendRule('suffix', [
                        'type' => 'div',
                        'class' => 'tips-info',
                        'domProps' => ['innerHTML' => $data['desc']]
                    ])->col(13);
    ...
    {
        $data['value'] = json_decode($data['value'], true) ?: '';
        $formbuser_ider[] = $this->builder->textarea($data['menu_name'], $data['info'], $data['value'])->placeholder($data['desc'])->appendRule('suffix', [
            'type' => 'div',
            'class' => 'tips-info',
            'domProps' => ['innerHTML' => $data['desc']]
        ])->rows(6)->col(13);
    ...
    public function createRadioForm(array $data, $control = [], $control_two = [], $control_three = [])
    {
        ...
        if ($parameter) {
            ...
            $formbuser_ider[] = $radio = $this->builder->radio($data['menu_name'], $data['info'], $res ? (int)$data['value'] : $data['value'])->options($options)->appendRule('suffix', [
                'type' => 'div',
                'class' => 'tips-info',
                'domProps' => ['innerHTML' => $data['desc']]
            ])->col(13);
    ...
    public function createUploadForm(int $type, array $data)
    {
        ...
        switch ($type) {
            case 1:
                ...
                $formbuser_ider[] = $this->builder->frameImage($data['menu_name'], $data['info'], $this->url(config('app.admin_prefix', 'admin') . '/widget.images/index', ['fodder' => $data['menu_name']], true), $data['value'])
            ...
            case 2:
                ...
                $formbuser_ider[] = $this->builder->frameImages($data['menu_name'], $data['info'], $this->url(config('app.admin_prefix', 'admin') . '/widget.images/index', ['fodder' => $data['menu_name'], 'type' => 'many', 'maxLength' => 5], true), $data['value'])
            ...
            case 3:
                $data['value'] = json_decode($data['value'], true) ?: '';
                if ($data['value'] != '') $data['value'] = set_file_url($data['value']);
                $formbuser_ider[] = $this->builder->uploadFile($data['menu_name'], $data['info'], $this->url('/admin/file/upload/1', ['type' => 1], false, true), $data['value'])
    ...
    public function createCheckboxForm(array $data)
    {
        ...
        if ($parameter) {
            ...
            $formbuser_ider[] = $this->builder->checkbox($data['menu_name'], $data['info'], $data['value'])->options($options)->appendRule('suffix', [
                'type' => 'div',
                'class' => 'tips-info',
                'domProps' => ['innerHTML' => $data['desc']]
            ])->col(13);
    ...
    public function createSelectForm(array $data)
    {
        ...
        if ($parameter) {
            ...
            $formbuser_ider[] = $this->builder->select($data['menu_name'], $data['info'], $data['value'])->options($options)->appendRule('suffix', [
                'type' => 'div',
                'class' => 'tips-info',
                'domProps' => ['innerHTML' => $data['desc']]
            ])->col(13);
    ...
    {
        $data['value'] = json_decode($data['value'], true) ?: '';
        $formbuser_ider[] = $this->builder->switches($data['menu_name'], $data['info'], $data['value'])->appendRule('suffix', [
            'type' => 'div',
            'class' => 'tips-info',
            'domProps' => ['innerHTML' => $data['desc']]
        ])->col(13);
    ...
    public function createConfigForm(int $tabId, array $relatedRule)
    {
        ...
        if (!$relatedRule) {
            $formbuser_ider = $this->createNoCrontrolForm($list);
        } else {
            $formbuser_ider = $this->createBindCrontrolForm($list, $relatedRule);
        }
        return $formbuser_ider;
    }
    ...
    public function createForm(array $list)
    {
        ...
        foreach ($list as $key => $data) {
            ...
            switch ($data['type']) {
                case 'text'://文本框
                    $formbuser_ider = array_merge($formbuser_ider, $this->createTextForm($data['input_type'], $data));
                    break;
                case 'radio'://单选框
                    $builder = [];
                    ...
                    $formbuser_ider = array_merge($formbuser_ider, $this->createRadioForm($data, $builder, $builder_two, $builder_three));
                    break;
                case 'textarea'://多行文本框
                    $formbuser_ider = array_merge($formbuser_ider, $this->createTextareaForm($data));
                    break;
                case 'upload'://文件上传
                    $formbuser_ider = array_merge($formbuser_ider, $this->createUploadForm((int)$data['upload_type'], $data));
                    break;
                case 'checkbox'://多选框
                    $formbuser_ider = array_merge($formbuser_ider, $this->createCheckboxForm($data));
                    break;
                case 'select'://多选框
                    $formbuser_ider = array_merge($formbuser_ider, $this->createSelectForm($data));
                    break;
                case 'switch'://开关
                    $formbuser_ider = array_merge($formbuser_ider, $this->createSwitchForm($data));
                    break;
            }
        }
        return $formbuser_ider;
        ...
        foreach ($list as $key => $data) {
            ...
            switch ($data['type']) {
                case 'text'://文本框
                    ...
                    break;
                case 'radio'://单选框
                    $formbuser_ider = array_merge($formbuser_ider, $this->createRadioForm($data));
                    break;
                case 'textarea'://多行文本框
                    $formbuser_ider = array_merge($formbuser_ider, $this->createTextareaForm($data));
                    break;
            ...
            if (isset($rv['son_type'])) {
                ...
                switch ($data['type']) {
                    ...
                    case 'radio'://单选框
                        ...
                        $formbuser_ider = array_merge($formbuser_ider, $this->createRadioForm($data, $son_builder));
                        break;
                    case 'textarea'://多行文本框
                        $formbuser_ider = array_merge($formbuser_ider, $this->createTextareaForm($data));
                        break;
    ...
    public function getConfigForm($url, int $tabId)
    {
        ...
        $list = $this->dao->getConfigTabAllList($tabId);
        $formbuser_ider = $this->createForm($list);
        $name = 'setting';
        if ($url) {
            $name = explode('/', $url)[2] ?? $name;
        }
        $postUrl = $this->postUrl[$name]['url'] ?? '/setting/config/save_basics';
        return create_form($title, $formbuser_ider, $this->url($postUrl), 'POST');
    }
    ...
    public function editConfigForm(int $id)
    {
        ...
        $service = app()->make(SystemConfigTabServices::class);
        $formbuser_ider = [];
        $formbuser_ider[] = $this->builder->input('menu_name', '字段变量', $menu['menu_name'])->disabled(1);
        $formbuser_ider[] = $this->builder->hidden('type', $menu['type']);
        [$configTabList, $data] = $service->getConfigTabListForm((int)($menu['config_tab_id'] ?? 0));
        $formbuser_ider[] = $this->builder->cascader('config_tab_id', '分类', $data)->options($configTabList)->filterable(true)->props(['props' => ['multiple' => false, 'checkStrictly' => true, 'emitPath' => false]])->style(['width' => '100%']);
        $formbuser_ider[] = $this->builder->input('info', '配置名称', $menu['info'])->autofocus(1);
        $formbuser_ider[] = $this->builder->input('desc', '配置简介', $menu['desc']);
        switch ($menu['type']) {
            case 'text':
                $menu['value'] = json_decode($menu['value'], true);
                $formbuser_ider[] = $this->builder->select('input_type', '类型', $menu['input_type'])->setOptions([
                ...
                //输入框验证规则
                $formbuser_ider[] = $this->builder->input('value', '默认值', $menu['value']);
                if (!empty($menu['required'])) {
                    $formbuser_ider[] = $this->builder->number('width', '文本框宽', (int)$menu['width']);
                    $formbuser_ider[] = $this->builder->input('required', '验证规则', $menu['required'])->placeholder('多个请用,隔开例如：required:true,url:true');
                }
                break;
            case 'textarea':
                $menu['value'] = json_decode($menu['value'], true);
                //多行文本
                if (!empty($menu['high'])) {
                    $formbuser_ider[] = $this->builder->textarea('value', '默认值', $menu['value'])->rows(5);
                    $formbuser_ider[] = $this->builder->number('width', '文本框宽', (int)$menu['width']);
                    $formbuser_ider[] = $this->builder->number('high', '多行文本框高', (int)$menu['high']);
                } else {
                    $formbuser_ider[] = $this->builder->input('value', '默认值', $menu['value']);
                }
                break;
            case 'radio':
                $formbuser_ider = array_merge($formbuser_ider, $this->createRadioForm($menu));
                //单选和多选参数配置
                if (!empty($menu['parameter'])) {
                    $formbuser_ider[] = $this->builder->textarea('parameter', '配置参数', $menu['parameter'])->placeholder("参数方式例如:\n1=>白色\n2=>红色\n3=>黑色");
                }
                break;
            case 'checkbox':
                $formbuser_ider = array_merge($formbuser_ider, $this->createCheckboxForm($menu));
                //单选和多选参数配置
                if (!empty($menu['parameter'])) {
                    $formbuser_ider[] = $this->builder->textarea('parameter', '配置参数', $menu['parameter'])->placeholder("参数方式例如:\n1=>白色\n2=>红色\n3=>黑色");
                }
                break;
            case 'upload':
                $formbuser_ider = array_merge($formbuser_ider, $this->createUploadForm(($menu['upload_type']), $menu));
                //上传类型选择
                if (!empty($menu['upload_type'])) {
                    $formbuser_ider[] = $this->builder->radio('upload_type', '上传类型', $menu['upload_type'])->options([['value' => 1, 'label' => '单图'], ['value' => 2, 'label' => '多图'], ['value' => 3, 'label' => '文件']]);
                }
                break;
            case 'switch':
                $formbuser_ider = array_merge($formbuser_ider, $this->createSwitchForm($menu));
                break;
        }
        $formbuser_ider[] = $this->builder->number('sort', '排序', (int)$menu['sort']);
        $formbuser_ider[] = $this->builder->radio('status', '状态', $menu['status'])->options([['value' => 1, 'label' => '显示'], ['value' => 0, 'label' => '隐藏']]);
        return create_form('编辑字段', $formbuser_ider, $this->url('/setting/config/' . $id), 'PUT');
    }
    ...
    public function createFormRule(int $type, int $tab_id): array
    {
        ...
        if ($form_type) {
            $formbuser_ider[] = $this->builder->hidden('type', $form_type);
            [$configTabList, $data] = $service->getConfigTabListForm((int)($tab_id ?? 0));
            $formbuser_ider[] = $this->builder->cascader('config_tab_id', '分类', $data)->options($configTabList)->filterable(true)->props(['props' => ['multiple' => false, 'checkStrictly' => true, 'emitPath' => false]])->style(['width' => '100%']);
            if ($info_type) {
                $formbuser_ider[] = $info_type;
            }
            $formbuser_ider[] = $this->builder->input('info', '配置名称')->autofocus(1);
            $formbuser_ider[] = $this->builder->input('menu_name', '字段变量')->placeholder('例如：site_url');
            $formbuser_ider[] = $this->builder->input('desc', '表单说明');
            $formbuser_ider = array_merge($formbuser_ider, $parameter);
            $formbuser_ider[] = $this->builder->number('sort', '排序', 0);
            $formbuser_ider[] = $this->builder->radio('status', '状态', 1)->options($this->formStatus());
        }
        return create_form('添加字段', $formbuser_ider, $this->url('/setting/config'), 'POST');
    }
