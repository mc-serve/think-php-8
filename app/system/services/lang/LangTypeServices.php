...
use app\system\dao\lang\LangTypeDao;
...
class LangTypeServices extends BaseServices
{
    ...
    public function langTypeSave(array $data)
    {
        if ($data['id']) {
            ...
            if ($res) {
                ...
                $list = $codeServices->selectList(['lang_type_id' => 1])->toArray();
                foreach ($list as $key => $item) {
                    unset($list[$key]['id']);
                    $list[$key]['lang_type_id'] = $res->id;
                }
    ...
    public function langTypeDel(int $id = 0)
    {
        ...
        $countryServices->update(['lang_type_id' => $id], ['lang_type_id' => 0]);
        /** @var LangCodeServices $codeServices */
        $codeServices = app()->make(LangCodeServices::class);
        $codeServices->delete(['lang_type_id' => $id]);
        $this->setDefaultLangName();
        return true;
    }
}
