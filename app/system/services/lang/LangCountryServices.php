...
namespace app\system\services\lang;

use app\system\dao\lang\LangCountryDao;
...
class LangCountryServices extends BaseServices
{
    ...
    public function LangCountryList(array $where = []): array
    {
        ...
        foreach ($list as &$item) {
            if (isset($langTypeList[$item['lang_type_id']])) {
                $item['link_lang'] = $langTypeList[$item['lang_type_id']]['language_name'] . '(' . $langTypeList[$item['lang_type_id']]['file_name'] . ')';
            } else {
                $item['link_lang'] = '暂无';
            }
        }
    ...
    public function langCountryForm($id)
    {
        ...
        $field[] = Form::select('lang_type_id', '关联语言', $info['lang_type_id'] ?? 0)->setOptions(Form::setOptions($setOption))->filterable(true)->appendRule('suffix', [
            'type' => 'div',
            'class' => 'tips-info',
