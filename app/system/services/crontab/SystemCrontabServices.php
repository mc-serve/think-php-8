...
namespace app\system\services\crontab;

use app\system\dao\crontab\SystemCrontabDao;
...
class SystemCrontabServices extends BaseServices
{
    ...
    public function getTimerList(array $where = [])
    {
        ...
        foreach ($list as &$item) {
            $item['next_execution_time'] = $item['next_execution_time'];
            $item['last_execution_time'] = $item['last_execution_time'] != 0 ? $item['last_execution_time'] : '暂未执行';
        }
