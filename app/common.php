...
use app\system\services\lang\LangTypeServices;
use app\system\services\lang\LangCodeServices;
use app\system\services\lang\LangCountryServices;
use think\facade\Config;
use think\facade\Log;
use think\facade\Db;

if (!function_exists('mcLog')) {
    /**
     * MC Log 日志
     * @param $msg
     * @author 扫地僧
...
if (!function_exists('get_mc_version')) {
    /**
     * 获取MC系统版本号
     * @param string $default
     * @return string
     */
    function get_mc_version($default = 'v1.0.0')
    {
        try {
    ...
    function getLang($code, array $replace = [])
    {
        ...
        try {
            ...
            $typeId = CacheService::remember('type_id_' . $range, function () use ($langCountryServices, $range) {
                return $langCountryServices->value(['code' => $range], 'lang_type_id') ?: 1;
            }, 3600);
            ...
            $lang = CacheService::remember($langStr, function () use ($typeId, $range, $langCodeServices) {
                return $langCodeServices->getColumn(['lang_type_id' => $typeId, 'is_admin' => 1], 'lang_explain', 'code');
            }, 3600);
            //获取返回文字
