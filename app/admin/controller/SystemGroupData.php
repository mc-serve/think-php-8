...
class SystemGroupData extends AuthController
{
    ...
    {
        [$system_group_id, $config_name] = $this->request->getMore([
            ['system_group_id', 0],
            ['config_name', '']
        ], true);
        if (!$system_group_id && !$config_name) return app('json')->fail(100100);
        if (!$system_group_id) {
            $system_group_id = $services->value(['config_name' => $config_name], 'id');
        }
        return app('json')->success($services->getGroupDataTabHeader($system_group_id));
    }
    ...
    public function index(SystemGroupServices $group)
    {
        ...
        if (!$where['system_group_id'] && !$where['config_name']) return app('json')->fail(100100);
        if (!$where['system_group_id']) {
            $where['system_group_id'] = $group->value(['config_name' => $where['config_name']], 'id');
        }
