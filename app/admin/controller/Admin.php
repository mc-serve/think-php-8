...
use app\system\services\SystemAuthServices;
use app\services\order\StoreOrderServices;
use app\services\product\product\StoreProductServices;
use app\services\product\product\StoreProductReplyServices;
use app\system\services\UpgradeServices;
...
class Admin extends AuthController
{
    ...
    public function auth()
    {
        ...
        // 正常域名
        $res = HttpService::request('http://authorize.mc-serve.com/api/auth_cert_query', 'post', [
            'domain_name' => $host,
            'label' => 34,
            'version' => $version
        ]);
...
//        $upgradeStatus = $upgradeServices->getUpgradeStatus();
        ...
        switch ((int)$status) {
            case 1:
                ...
                try {
                    ...
                    if ($services->count(['menu_name' => 'cert_mc'])) {
                        $services->update(['menu_name' => 'cert_mc'], ['value' => json_encode($autoContent . ',' . $authCode)]);
                    } else {
                        $services->save([
                            'menu_name' => 'cert_mc',
                            'type' => 'text',
    ...
    public function saveCopyright()
    {
        ...
        /** @var SystemConfigServices $services */
        $services = app()->make(SystemConfigServices::class);
        if ($services->count(['menu_name' => 'nncnL_mc_copyright'])) {
            $services->update(['menu_name' => 'nncnL_mc_copyright'], ['value' => json_encode($copyright)]);
        } else {
            $services->save([
                'menu_name' => 'nncnL_mc_copyright',
                'type' => 'text',
                'input_type' => 'input',
                'config_tab_id' => 1,
        ...
        if ($services->count(['menu_name' => 'nncnL_mc_copyright_image'])) {
            $services->update(['menu_name' => 'nncnL_mc_copyright_image'], ['value' => json_encode($copyrightImg)]);
        } else {
            $services->save([
                'menu_name' => 'nncnL_mc_copyright_image',
                'type' => 'text',
                'input_type' => 'input',
                'config_tab_id' => 1,
