...
class SystemCity extends AuthController
{
    ...
    public function save()
    {
        ...
        $this->validate($data, \app\admin\validate\setting\SystemCityValidate::class, 'save');
        if ($data['parent_id'] == 0) {
        ...
        if ($data['id'] == 0) {
            ...
            $data['system_city_id'] = intval($this->services->getCityIdMax() + 1);
    ...
    {
        return app('json')->success($this->services->fullList('parent_id,name as label,system_city_id as value'));
    }
}
